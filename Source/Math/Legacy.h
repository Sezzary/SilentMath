...
namespace Silent::Math
{
    ...
    /** @brief Constructs a `VECTOR3` in a fixed-point Q19.12 format for RODATA vectors used as inlined function
     * parameters.
     *
     * @param x X component (`float`).
     * @param y Y component (`float`).
     ...
     *
     * TODO: Could this have been a more abstract macro which was garbled by optimizations taking advantage of 1
    ...
    #define Math_SetSVectorFast(vec, x, y, z)                         \
        *(int*)&((vec)->vx) = (int)((x) & 0xFFFF) | (int)((y) << 16); \
        *(short*)&((vec)->vz) = (z)
    ...
    /** @brief Normalizes Q19.12 fixed-point degrees, unsigned integer range `[0, 4096]` to the signed integer range
     * `[-2048, 2047]`.
