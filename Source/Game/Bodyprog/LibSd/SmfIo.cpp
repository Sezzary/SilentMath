...
#include "Game/Bodyprog/LibSd/SmfTables.h"
...
namespace Silent::Game
{
    ...
    void key_on(u8 chan, u8 c1, u8 c2) // 0x800A5158
    {
        ...
        {
            vadId = smf_song[chan >> 4].sd_seq_vab_id_508;
        }
        else
        {
            vadId = midi->bank_change_5A;
        }

        vab0 = vab_h[vadId].vh_addr_4;
        vab1 = vab_h[vadId].vh_addr_4;
        sp28 = 0;
        ...
        for (tone = 0; tone < prog->tones; tone++)
        {
            ...
            if (sd_vag_atr->vag != 0 && c1 >= sd_vag_atr->min && sd_vag_atr->max >= c1)
            {
                ...
                for (i = 0, vag_addr = 0, addr_p = (u16*)((u8*)vab_h[vadId].vh_addr_4 + (vab0->vab_h.ps * 512) + 2080);
                     i < sd_vag_atr->vag;
                     addr_p++, i++)
                {
                    vag_addr += *addr_p;
                ...
                s_attr.voice         = spu_ch_tbl[vo];
                vag_addr             = vag_addr * 8;
                s_attr.addr          = vab_h[vadId].vb_start_addr_10 + vag_addr;
