...
namespace Silent::Game
{
                                        ...
                                        q19_12 accel, q19_12 total_max_spd, q19_12 dec_forwd_lim_spd, q19_12 dec_accel_side) // 0x80048F28
    {
        ...
        cam2tgt_ang_y = Math_Ratan2(tgt_pos->vx - now_pos->vx, tgt_pos->vz - now_pos->vz);
        ...
        *velo_z += Q12_MULT(add_spd, Math_Cos(cam2tgt_ang_y));
        ...
        if (total_max_spd < lim_spd)
        {
            ...
            ang_y    = Math_Ratan2(*velo_x, *velo_z);
    ...
    void vwMatrixToAngleYXZ(SVECTOR* ang, const MATRIX* mat) // 0x800495D4
    {
        ...
        r_xz    = SquareRoot0((mat->m[0][2] * mat->m[0][2]) +
                              (mat->m[2][2] * mat->m[2][2]));
        ang->vx = Math_Ratan2(-mat->m[1][2], r_xz);

        if (ang->vx == Q12_ANGLE(90.0f))
        {
            ang->vz = Q12_ANGLE(0.0f);
            ang->vy = Math_Ratan2(mat->m[0][1], mat->m[2][1]);
        }
        ...
        {
            ang->vz = Math_Ratan2(mat->m[1][0], mat->m[1][1]);
            ang->vy = Math_Ratan2(mat->m[0][2], mat->m[2][2]);
        }
    }
    ...
    void Vw_CoordHierarchyMatrixCompute(GsCOORDINATE2* rootCoord, MATRIX* viewMat) // 0x80049984
    {
    ...
    bool Vw_AabbVisibleInScreenCheck(s32 minX, s32 maxX, s32 minY, s32 maxY, s32 minZ, s32 maxZ) // 0x80049D04
    {
        ...
        for (i = 0; i < BOX_VERTEX_COUNT; i++)
        {
            ...
            screenDepth = RotTransPers(&vertOffset, (int*)&screenPos, &depthDmy, &depthDmy) - 1;
    ...
    bool Vw_AabbVisibleInFrustumCheck(MATRIX* modelMat, s16 minX, s16 minY, s16 minZ, s32 maxX, s32 maxY, s32 maxZ, u16 nearPlane, u16 farPlane) // 0x80049F38
    {
        ...
        {
            transformedZ = 4;
            transformedZ = RotTransPers(&cullData->field_20[i], (int*)&screenPos, &cullData->field_178, &cullData->field_178) * transformedZ;
        ...
        if (Vw_ScreenRegionSpanCheck(&regionFlags) != 1)
        {
            for (i = 0; i < 8; i++)
            {
                RotTrans(&cullData->field_20[i], (VECTOR*)&cullData->field_60[i], &cullData->field_178);
            }
    ...
    q19_12 vwVectorToAngle(SVECTOR* ang, const SVECTOR* vec) // 0x8004A714
    {
        ...
        ang->vx = Math_Ratan2(-vec->vy, SquareRoot0(localVec.vx + localVec.vz));
        ang->vy = Math_Ratan2(vec->vx, vec->vz);
        ang->vz = Q12_ANGLE(0.0f);
        return ret_r;
    }
