...
Converts a `.XA` (audio) or `.STR` (video) file to a usable format.
"""
...
def _convert_xa_to_wav(ffmpeg_cmd: str, output_folder: Path, xa_file: Path):
    ...
    :param xa_file: The source `.XA` file to process.
    """
    new_file = output_folder / f"{xa_file.stem}.WAV"
...
def _convert_str_to_mpg(ffmpeg_cmd: str, output_folder: Path, str_file: Path):
    ...
    :param str_file: The source `.STR` file to process.
    """
    newFile = output_folder / f"{str_file.stem}.MPG"
